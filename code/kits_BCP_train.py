...
from utils.losses import DiceLoss, mask_DiceLoss
...
from utils.BCP_utils import mix_loss, update_ema_teacher, update_ema_variables, kits_loss

parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='../data_split/kits19', help='Name of Dataset')
...
parser.add_argument('--max_samples', type=int, default=160, help='maximum samples to train')
...
# def get_pseudo_label(out, thres=0.5):
#     probs = F.softmax(out, dim=1)
#     _, probs = torch.max(probs, dim=1)
#     # probs = out.max(dim=1)[1]
#     return probs
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # if iter_num % 200 == 0:
            #     model.eval()
            #     dice_sample = test_3d_patch.var_all_case_kits(model, num_classes=num_classes, patch_size=patch_size,
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            update_ema_variables(model, ema_model, 0.999)
            ...
            if iter_num % 200 == 0:
                model.eval()
                dice_sample = test_3d_patch.var_all_case_kits(model, num_classes=num_classes,
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./model/BCP/kits_{}_{}_labeled/pre_train".format(args.exp, args.labelnum)
    self_snapshot_path = "./model/BCP/kits_{}_{}_labeled/self_train".format(args.exp, args.labelnum)
    ...
    shutil.copy('../code/kits_BCP_train.py', self_snapshot_path)
